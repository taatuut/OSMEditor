$(document).ready(function(){
	...
	map.setView(new L.LatLng(viewlat, viewlon), 12);                
	var heatmap = new L.TileLayer.HeatCanvas({},{'step':0.1, 'degree':HeatCanvas.LINEAR, 'opacity':0.7});
	var data;
	...
	data = $(xml).find("node").each(function() {
		$("#hdata").append("["+$(this).attr("lat")+","+$(this).attr("lon")+","+($(this).children("tag").length^3).toString()+"],");
	});
